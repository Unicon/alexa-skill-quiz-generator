...
import static org.unicon.lex.utils.MediaUtil.Type.VIDEO;

public class PlayVideoService extends AbstractLexService {
    ...
    public MessageContent getAnswer(LexRequest request) {
        ...
        if (url != null) {
            try {
                html = MediaUtil.createMediaElement(url, VIDEO);
            } catch (Exception e) {
        ...
        if (html == null) {
            content = new DefaultMessageContent(format("Sorry there is no video for '%s' in '%s'", word, subject));
        } else {
